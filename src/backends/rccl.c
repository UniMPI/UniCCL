...
/* Opaque 128-byte unique id (type shared with the fixtures via
 * unicc_native_id.h; see the nccl.c comment for the sanitizer rationale). */

static int (*s_rccl_get_unique_id)(unicc_native_uid_t *uid);
...
int unicc_vtable_init_rccl(unicc_lib_handle_t handle) {
    ...
    s_rccl_comm_init_rank = (int(*)(unicc_comm_t*, int, unicc_native_uid_t, int))
    ...
    s_rccl_get_unique_id = (int(*)(unicc_native_uid_t*))
